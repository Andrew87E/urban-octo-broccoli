...
const getLocationData = () => {
  ...
  if (initialData && citiesData) {
    ...
    // console.log(JSON.parse(citiesData));
...
const getCapitalIntro = async (capital, state) => {
  const response = await fetch(
    `https://en.wikipedia.org/api/rest_v1/page/summary/${capital}`
  );
  const data = await response.json();
  console.log(data.extract); // Log for debugging
  if (data.extract.includes("refers to:")) {
    const response = await fetch(
      `https://en.wikipedia.org/api/rest_v1/page/summary/${capital},${state}`
    );
    const data = await response.json();
    console.log(data.extract); // Log for debugging
    if (data.extract.includes("refers to:")) {
...
const updateContent = async () => {
  ...
  const capitalIntro = await getCapitalIntro(
    citiesData.capital,
    initialData.state.name
  );
  updateCapitalIntro(capitalIntro.intro);
...
const updatePhoto = async (photo) => {
  const photoEl = document.getElementById("capital-pic");
  photo = photo.replace(/ /g, "_");
  photoEl.src = photo;
};

const updateCapitalIntro = async (intro) => {
  const stateIntroEl = document.getElementById("capital-intro");
